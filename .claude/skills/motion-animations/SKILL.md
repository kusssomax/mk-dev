...
**Current Version**: 13.0.0 (verified 2026-08-06)

**⚠️ Breaking Change in 13.0.0** (upgrading from any 12.x):
- Removed the optional `@emotion/is-prop-valid` dependency. If you use styled-components or Emotion with `motion(styled.div)`-style wrapping, DOM props that were previously auto-filtered may now leak to the DOM.
- **Fix**: explicitly inject the validator via `MotionConfig`:
```tsx
...
```
- If you don't use styled-components/Emotion (e.g. Tailwind-only projects), this change has no effect on you.
...
**`mode` prop** (default: `"sync"`) — controls how entering/exiting children coordinate:
```tsx
<AnimatePresence mode="wait">
  {/* "sync" (default): enter and exit animate simultaneously */}
  {/* "wait": entering child waits for exiting child to finish first */}
  {/* "popLayout": exiting element is removed from layout flow immediately,
      so surrounding elements reflow without waiting — ideal for carousels/lists */}
</AnimatePresence>
```
Other props: `initial` (skip animation on first mount), `onExitComplete`, `propagate` (let nested AnimatePresence/exit props bubble up), `custom` (pass data to exit variants).
...
- `layoutDependency`: Pass a value that must change before Motion re-measures layout — skips measurement work when unrelated re-renders happen
- `layoutAnchor`: Customize the animation's origin point with `0`-`1` progress values on x/y
...
Use the `stagger()` helper with `delayChildren` on a parent's `transition` to stagger children's animations:
...
<motion.ul variants={container} initial="hidden" animate="show">
  {items.map(item => (
    <motion.li key={item.id} variants={{ hidden: { opacity: 0, y: 10 }, show: { opacity: 1, y: 0 } }} />
  ))}
</motion.ul>
```

`stagger(duration, options)` accepts `from: "first" | "last" | "center" | number` to control where the stagger originates, plus `ease` for the delay curve itself. This is the current recommended pattern for staggered reveals (replaces manually calculating per-child `delay` values).
...
## Package Versions (Verified 2026-08-06)

| Package | Version | Status |
|---------|---------|--------|
| motion | 13.0.0 | ✅ Latest stable (released 2026-08-05 — see breaking change note in Installation) |
| framer-motion | 13.0.0 | ✅ Same version as motion |
| react | 19.2.8 | ✅ Latest stable |
| next | 16.3.0 | ✅ Latest stable |
| vite | — | Not independently re-verified this pass; check `npm view vite version` before relying on it |
